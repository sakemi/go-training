...
func ParseMin(input string) (_ Expr, err error) {
	nums := map[int][]rune{}
	i := 0
	prev := false
	minus := false
	...
	f := []float64{}
	for _, runes := range nums {
		if runes[len(runes)-1] == '.' {
			runes = runes[:len(runes)-1]
		}
		num, _ := strconv.ParseFloat(string(runes), 64)
		f = append(f, num)
	}
	if len(f) == 0 {
		return nil, fmt.Errorf("No operand.")
	}
	return min(f), nil
}
